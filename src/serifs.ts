...
*/
	...
	noting: {
		...
		talk : item => `みなさんで「${item}」の事について話してみるのはどうですか？`,
		choice : item => `せっかくだから、私はこの「${item}」を選びます！`,
		need : item => `みなさんが今欲しいものをあててみましょう……「${item}」でしょうか？`,
		brake : item =>	`なにもしていないのに、${item}がだめになっちゃいました…`,
		summon : item => `むむむむっ…「${item}」が召喚できました`,
		offering : item => `サーバーに${item}をお供えしておきますね`,
		give: item => `${item}、タイムラインのみなさんで分けてくださいね`,
		want: item => `${item}、欲しいなぁ…`,
		see: item => `お散歩していたら、道に${item}が落ちているのを見たんです！`,
		expire: item => `気づいたら、${item}の消費期限がきれちゃってました…`,
	},
/**季節ごとのセリフ */
	seasons: {
		spring: [
